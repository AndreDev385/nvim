...
return {
  ...
  before_init = function(_, config)
    if config.init_options and config.init_options.typescript and not config.init_options.typescript.tsdk then
      local tsdk = vim.g.tsdk
      if tsdk then
        config.init_options.typescript.tsdk = tsdk
      elseif config.root_dir then
        local tsconfig = vim.fs.find('tsconfig.json', { path = config.root_dir, upward = true })[1]
        if tsconfig then
          local tsdk_path = vim.fs.dirname(vim.fs.find('typescript/lib/tsserver.js', { path = vim.fs.dirname(tsconfig) })[1])
          if tsdk_path then
